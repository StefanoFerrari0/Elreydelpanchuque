...
function ProductID( { data }) {
  ...
  const product = data; 
    return (
    <Layout title={`${product.title} | El rey del panchuque`} 
    ...
    link={`${process.env.BASE_URL}/products/${product._id}`}>
      <article className={styles.productContainer}>
        ...
        <div className={styles.col}>
          ...
          <p className={styles.note}>El precio publicado es de contado efectivo / Transferencia bancaria  / Mercado Pago. Con tarjeta de crédito tiene interés</p>
